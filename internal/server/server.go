...
func(ss *serverSession)pump(id uint32,d net.Conn){buf:=make([]byte,32*1024);for{n,err:=d.Read(buf);if n>0{if we:=ss.write(protocol.Frame{Type:protocol.TypeData,StreamID:id,Payload:append([]byte(nil),buf[:n]...)});we!=nil{ss.closeStream(id);return}};if err!=nil{ss.closeStream(id);return}}}
