...
type serverSession struct { c net.Conn; r *bufio.Reader; writeMu sync.Mutex; mu sync.Mutex; streams map[uint32]net.Conn; send *protocol.BlockingFlowWindow; streamSend map[uint32]*protocol.BlockingFlowWindow }
...
func(ss *serverSession)reset(id uint32){ss.mu.Lock();d:=ss.streams[id];delete(ss.streams,id);delete(ss.streamSend,id);ss.mu.Unlock();if d!=nil{_=d.Close()};_=ss.write(protocol.Frame{Type:protocol.TypeReset,StreamID:id})}
func(ss *serverSession)closeStream(id uint32){ss.mu.Lock();d:=ss.streams[id];delete(ss.streams,id);delete(ss.streamSend,id);ss.mu.Unlock();if d!=nil{_=d.Close()};_=ss.write(protocol.Frame{Type:protocol.TypeClose,StreamID:id})}
func(ss *serverSession)closeAll(){ss.mu.Lock();for id,d:=range ss.streams{_=d.Close();delete(ss.streams,id);delete(ss.streamSend,id)};ss.mu.Unlock();_=ss.c.Close()}
