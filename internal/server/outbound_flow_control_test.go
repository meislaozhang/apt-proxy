...
func TestPumpWaitsForSessionWindow(t *testing.T) {
    ...
    if err := sessionWindow.Acquire(nilContext{}, 1); err != nil { t.Fatal(err) }
    ...
    _, err = r.ReadByte()
    if err == nil { t.Fatal("DATA reached wire before session WINDOW_UPDATE") }
    _ = peer.SetReadDeadline(time.Time{})

    if err := sessionWindow.Add(uint64(len(payload))); err != nil { t.Fatal(err) }
    ...
    _ = originPeer.Close()
    select {
    case f := <-readFrameAsync(r):
        if f.err != nil { t.Fatalf("read half-close frame: %v", f.err) }
        if f.frame.Type != protocol.TypeHalfClose || f.frame.StreamID != streamID {
            t.Fatalf("unexpected origin close frame: %+v", f.frame)
        }
    case <-time.After(time.Second):
        t.Fatal("origin half-close frame was not delivered")
    }
    select {
    case <-pumpDone:
    case <-time.After(time.Second):
...
type frameResult struct { frame protocol.Frame; err error }
func readFrameAsync(r *bufio.Reader) <-chan frameResult {
    ch := make(chan frameResult, 1)
    go func() { f, err := protocol.ReadFrame(r); ch <- frameResult{frame: f, err: err} }()
    return ch
}
