...
func(st *stream)Read(p []byte)(int,error){st.readMu.Lock();defer st.readMu.Unlock();for len(st.pending)==0{select{case b:=<-st.data:st.pending=b;case <-st.closed:return 0,io.EOF;case <-st.sess.done:return 0,io.EOF}};n:=copy(p,st.pending);st.pending=st.pending[n:];return n,nil}
...
// CloseWrite performs a protocol-level half-close of the local write direction.
...
// Reset aborts the stream without closing the parent session.
